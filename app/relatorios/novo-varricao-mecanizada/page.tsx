...
export default function NovoVarricaoMecanizadaPage() {
  const [assunto] = useState("Varrição Mecanizada");
  ...
  async function save() {
    ...
    try {
      ...
      try {
        ...
        const idRetry = uuidv4();
        const dataInicioISORetry = dataInicio.toISOString().split('T')[0];
        const dataFimISORetry = dataFim.toISOString().split('T')[0];
        
        const relRetry: RegistroRelatorio = {
          id: idRetry,
          title: assunto,
          tipoServico: "VARRICAO_MECANIZADA",
          assunto: assunto,
          dataInicio: dataInicioISORetry,
          dataTermino: dataFimISORetry,
          sub: sub,
          local: local,
          descricao: descricao,
          fotos: fotos.map(url => ({ url, descricao: assunto })),
        ...
        <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold text-zinc-900 dark:text-zinc-100">
              Novo Relatório - Varrição Mecanizada
            </h1>
            <p className="mt-2 text-zinc-600 dark:text-zinc-400">
              Cadastre um novo relatório de varrição mecanizada
            </p>
          </div>
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-xl shadow-lg p-6 space-y-6">
            ...
            <div>
              ...
              <textarea
                ...
                placeholder="Descreva os detalhes do serviço de varrição mecanizada..."
                rows={4}
                className="w-full px-3 py-2 border border-zinc-300 dark:border-zinc-600 rounded-lg bg-white dark:bg-zinc-700 text-zinc-900 dark:text-zinc-100 focus:ring-2 focus:ring-green-500 focus:border-transparent"
              />
            </div>
