...
<body>
    ...
    <div class="page cover">
        ...
        <h2>${(rel as any).sub}</h2>
        <div class="date">${formatDateForCover((rel as any).dataInicio || (rel as any).data)}</div>
    ...
    <div class="page content-page">
        ...
        <div class="info-grid">
            ...
            <div class="info-item">
                <h3>Local</h3>
                <p>${(rel as any).local || 'Não informado'}</p>
            </div>
            <div class="info-item">
                <h3>Nome do Evento</h3>
                <p>${(rel as any).nomeEvento || 'Não informado'}</p>
            </div>
        </div>
        
        ${(rel as any).descricao ? `
        <div class="description">
            <h3>Descrição</h3>
            <p>${(rel as any).descricao}</p>
        </div>
        ` : ''}
        
        ${(rel as any).fotos && (rel as any).fotos.length > 0 ? `
        <div class="photos-grid">
            ${(rel as any).fotos.map((foto: any) => `
                <div class="photo-item">
                    <img src="${foto.url}" alt="Foto do evento">
                    <div class="photo-caption">${(rel as any).nomeEvento || 'Evento'}</div>
                </div>
            `).join('')}
        </div>
        ` : `
...
<body>
    ...
    <div class="page cover-page">
        ...
        <div class="cover-content">
            <h1 class="cover-title">RELATÓRIO DE EVIDÊNCIAS</h1>
             <div class="cover-date">${formatDateForCover(rel.tipoServico === 'DDS' ? (rel as any).dataInicio : ('data' in rel ? rel.data : (rel as any).dataInicio))}</div>
        </div>
    </div>
    ...
    <div class="page service-page">
        ...
        <div class="service-subregion">     <h2>EVIDÊNCIAS CONSOLIDADAS</h2>
</div>
